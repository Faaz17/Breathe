...
Breathe captures your meeting tab's audio, transcribes it **on your own machine** with a local Whisper model, and — only when you ask — summarises the transcript into Decisions, Action Items, and Open Questions using your own free Groq API key.
...
Breathe isn't on the Chrome Web Store — you load it as an unpacked extension. Two ways:
...
| Meeting sites only | Host access is limited to `meet.google.com`, `*.zoom.us/wc/*`, `*.webex.com` — never all sites |
...
Local dev setup, the toolchain matrix, and the reload workflow are in [docs/SETUP.md](docs/SETUP.md). Architecture and design rationale live in [.claude/CLAUDE.md](.claude/CLAUDE.md).
